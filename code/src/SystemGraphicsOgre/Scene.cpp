...
OGREGraphicsScene::OGREGraphicsScene(ISystem* pSystem)
	...
	if( m_bUseInstancedGeom )
	{
		...
		for ( itMap = m_InstancedGeoms.begin(); itMap != m_InstancedGeoms.end(); itMap++ )
		{
			for( u32 i=0; i < itMap->second.size(); i++ )
			{
				...
				// TODO: ALEX HAS MADE THIS CHANGE TEST IT!
				unsigned short objCount = 0; // will be ignored by getObjectsAsArray

				Ogre::InstancedGeometry::BatchInstance::InstancedObjectIterator iter = pBatchInst->getObjectIterator();
				...
				Ogre::InstancedGeometry::InstancedObject** array=new Ogre::InstancedGeometry::InstancedObject*[objCount];

				iter = pBatchInst->getObjectIterator();
				unsigned j=0;
				while(iter.hasMoreElements())
				{
					array[j]=iter.current()->second;
					j++;
				}

				mapObjArray[ itMap->first ].push_back( array );
				///////////////////////////////////////////////////////////////////////////////////
			}
		}
...
void OGREGraphicsScene::GlobalSceneStatusChanged(GlobalSceneStatus Status)
{
	...
	if( Status == GlobalSceneStatus::PostLoadingObjects )
	{
		...
		if( m_bUseInstancedGeom )
		{
			...
			for ( ObjectsList::iterator it = m_Objects.begin(); it != m_Objects.end(); it++ )
			{
				// Tell MyGUI about the viewport to render to
				if( ((*it)->GetType() == OGREGraphicsObject::Type_Camera))
					m_Platform->getRenderManagerPtr()->setActiveViewport(0);
...
Error OGREGraphicsScene::Initialize(std::vector<Properties::Property> Properties)
{
	...
	//
	m_pOverlay = m_pOverlayManager->create("StatsOverlay");
	m_pOverlay->show();

	//-------------------------------------------------------------------------
	// Set up MyGUI for this scene
	m_Platform = new MyGUI::OgrePlatform();
	m_Platform->initialise(reinterpret_cast<OGREGraphicsSystem*>(m_pSystem)->GetOGRERenderWindow(), m_pSceneManager);
	m_GUI = new MyGUI::Gui();
	m_GUI->initialise("MyGUI_Core.xml");

	MyGUI::VectorWidgetPtr root = MyGUI::LayoutManager::getInstance().loadLayout("GameHUD.layout");
	for(MyGUI::VectorWidgetPtr::iterator iter = root.begin(); iter != root.end(); iter++)
	{
		if((*iter)->getName() == "tbTimer")
		{
			(*iter)->castType<MyGUI::TextBox>()->setCaption("34");
		}
	}
...
pcstr* OGREGraphicsScene::GetObjectTypes(void)
{
	return OGREGraphicsObject::sm_kapszTypeNames;
}


ISystemObject* OGREGraphicsScene::CreateObject(pcstr pszName, pcstr pszType)
{
	ASSERT( m_bInitialized );
...
Error OGREGraphicsScene::DestroyObject(ISystemObject* pSystemObject)
{
	ASSERT( m_bInitialized );
...
ISystemTask* OGREGraphicsScene::GetSystemTask(void)
{
	return m_pTask;
}


System::Changes::BitMask OGREGraphicsScene::GetPotentialSystemChanges(void)
{
	return System::Changes::None;
}


const void* OGREGraphicsScene::GetSystemChangeData(System::Change SystemChange)
{
