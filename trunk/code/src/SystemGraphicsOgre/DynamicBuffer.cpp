...
void DynamicBuffer::setOperationType(Ogre::RenderOperation::OperationType opType)
{
	mRenderOp.operationType = opType;
}

Ogre::RenderOperation::OperationType DynamicBuffer::getOperationType() const
{
...
Ogre::Real DynamicBuffer::getSquaredViewDepth(const Ogre::Camera* pCam) const
{
	...
	Ogre::Matrix4 mWorld;
	getWorldTransforms(&mWorld);

	Ogre::Vector3 vWorld = Ogre::Vector3(mWorld[3][0], mWorld[3][1], mWorld[3][2]);
